...
int main() {
    ...
    DatarefCategory claimed_category = DatarefCategory::kSystems;
    ...
    callbacks.on_claim_ownership = [&](DatarefCategory category) {
    ...
    const std::string cmd5 = "CLAIM_OWNERSHIP engine\n";
    companion.SendTo("127.0.0.1", kControlUdpPort, cmd5.data(), cmd5.size());
    PumpFor(listener, 200ms);
    assert(ownership_claimed);
    assert(claimed_category == DatarefCategory::kEngine);
    std::printf("CLAIM_OWNERSHIP parsed correctly: category=%d\n", static_cast<int>(claimed_category));
    ...
    const std::string cmd6 = "CLAIM_OWNERSHIP bogus\n";
    ...
    // renders as the ownership buttons' state. Both OwnershipUiState
    // values in one push, to cover each one.
    listener.SetSharedCockpitOwnership({/*systems=*/ControlListener::OwnershipUiState::kMe,
                                         ...
                                         /*avionics=*/ControlListener::OwnershipUiState::kMe});
    ...
    for (int i = 0; i < 50 && !got_ownership; ++i) {
        ...
        if (received > 0) {
            buf[received] = '\0';
            std::string msg(buf, static_cast<size_t>(received));
            if (msg.find("SHARED_COCKPIT_OWNERSHIP systems:me engine:peer avionics:me") != std::string::npos) {
                got_ownership = true;
            }
        }
        std::this_thread::sleep_for(10ms);
    ...
    std::printf("SHARED_COCKPIT_OWNERSHIP correctly pushed (me/peer)\n");
